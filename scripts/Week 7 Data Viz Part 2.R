...
# _b = binned scale, but colors still kind of continuous
...
#_d = discrete, colors are not super continuous
...
# color = __ means we color the outline, fill = __ means we fill with that color
# if we do both color= __ & fill= __, it'll do both (but ugly)
# so, if you add in the alpha, it will look look again
...
ggplot(diamonds, aes(x = clarity, fill = cut)) + 
  ...
  theme_classic()
# but this can be kind of complex to interpret
...
display.brewer.all(colorblindFriendly = TRUE)
# can use a bunch of different shapes, number isn't reated to size at all
